...
feature -- Basic operations
	...
	twister_32
		...
		do
			...
			divider (rng.generating_type + "  1000 {NATURAL_32} numbers")
			test_1000 (rng, directory + "mt19937ar.out")
				-- {REAL_64}
			create rng.from_array (init_array)
...
feature {NONE} -- Implementation
	...
	test_constrained (a_rng: TWISTER_32; a_lower, a_upper: NATURAL_32)
		...
		local
			...
			n: NATURAL_32
			c: INTEGER_32
			hist: HASH_TABLE [INTEGER_32, NATURAL_32]	-- [count, key]
	...
	test_1000 (a_rng: TWISTER_32; a_filename: STRING_8)
		...
		do
			...
			if not failed then
				...
				f.read_line
				check
					expected_line: f.last_string ~ "1000 outputs of genrand_int32()"
	...
	test_reals (a_rng: TWISTER_32; a_filename: STRING_8)
...
--				io.put_string ("  after read_line   last_string = '" + f.last_string + "' %N")
				check
					expected_line: f.last_string ~ "1000 outputs of genrand_real2()"
						-- because that is what is in the file
				end
				io.put_string ("  -- {REAL_32} checks use `very_close' from {SAFE_DOUBLE_MATH) --%N")
...
feature {NONE} -- Implementation
	...
	valid_types: ARRAYED_SET [TYPE [detachable ANY]]
		...
		once ("OBJECT")
			...
			Result.extend ((create {TWISTER_32}).generating_type)
		end

	init_array: ARRAY [NATURAL_32]
			-- Array used in 32-bit generators in `from_array'.
		once
			Result :=  {ARRAY [NATURAL_32]} <<0x123, 0x234, 0x345, 0x456>>
		end
